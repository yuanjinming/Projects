...
public class ResultMapElementGenerator extends
    ...
    public void addElements(XmlElement parentElement) {
        ...
        if (context.getPlugins().sqlMapResultMapElementGenerated(answer,
                        introspectedTable)) {
    ...
    private void addResultMapElements(XmlElement answer) {
    	...
    	IntrospectedColumn column = introspectedTable.getPrimaryKeyColumn();
        resultElement
                .addAttribute(new Attribute(
                        "column", MyBatis3FormattingUtilities.getRenamedColumnNameForResultMap(column))); //$NON-NLS-1$
        resultElement.addAttribute(new Attribute(
                "property", column.getJavaProperty())); //$NON-NLS-1$
        resultElement.addAttribute(new Attribute("jdbcType", //$NON-NLS-1$
        		column.getJdbcTypeName()));

        if (stringHasValue(column.getTypeHandler())) {
            resultElement.addAttribute(new Attribute(
                    "typeHandler", column.getTypeHandler())); //$NON-NLS-1$
        }
    ...
    private void addResultMapConstructorElements(XmlElement answer) {
        ...
        columns = introspectedTable.getBaseColumns();
        for (IntrospectedColumn introspectedColumn : columns) {
            resultElement = new XmlElement("arg"); //$NON-NLS-1$
